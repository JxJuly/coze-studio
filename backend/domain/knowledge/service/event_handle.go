...
import (
	...
	progressbar_contract "github.com/coze-dev/coze-studio/backend/infra/contract/document/progressbar"
...
func (k *knowledgeSVC) processDocumentChunks(ctx context.Context,
	...
	for i := 0; i < len(parseResult); i += batchSize {
		if i < int(cacheRecord.LastProcessedNumber) {
			if err := progressbar.AddN(batchSize * 2); err != nil {
...
func (k *knowledgeSVC) batchProcessSlice(ctx context.Context, doc *entity.Document,
	...
	progressBar progressbar_contract.ProgressBar) error {
	...
	if doc.Type == knowledge.DocumentTypeTable && !cacheRecord.RecordToRDB {
	...
	// Store slices in database if not already done
	if !cacheRecord.RecordToDB {
		if err := k.storeSlicesInDB(ctx, doc, parseResult, startIdx, ids); err != nil {
			return err
		}
		cacheRecord.RecordToDB = true
		if err := k.recordIndexDocumentStatus(ctx, cacheRecord,
			fmt.Sprintf(indexDocCacheKey, doc.KnowledgeID, doc.ID)); err != nil {
			return err
		}
	...
	cacheRecord.RecordToDB = false
	cacheRecord.RecordToRDB = false
	cacheRecord.RecordToES = false
	cacheRecord.RecordToVectorDB = false
	...
	// Mark slices as done
	return k.sliceRepo.BatchSetStatus(ctx, ids, int32(model.SliceStatusDone), "")
}
...
func (k *knowledgeSVC) indexSlicesInSearchStores(ctx context.Context, doc *entity.Document,
	...
	for _, manager := range k.searchStoreManagers {
		if (manager.GetType() == searchstore.TypeTextStore && cacheRecord.RecordToES) ||
			(manager.GetType() == searchstore.TypeVectorStore && cacheRecord.RecordToVectorDB) {
			continue
		}
		...
		// Update cache record based on store type
		if manager.GetType() == searchstore.TypeTextStore {
			cacheRecord.RecordToES = true
		} else {
			cacheRecord.RecordToVectorDB = true
		}
